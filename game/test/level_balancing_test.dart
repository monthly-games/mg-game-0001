...
void main() {
  ...
  test('difficulty, waves, and rewards grow without regressions', () {
    for (var i = 1; i < kLevelDesign.length; i += 1) {
      expect(
        kLevelDesign[i].difficulty,
        greaterThan(kLevelDesign[i - 1].difficulty),
      );
      expect(kLevelDesign[i].wave, greaterThan(kLevelDesign[i - 1].wave));
      expect(
        kLevelDesign[i].goldReward,
        greaterThan(kLevelDesign[i - 1].goldReward),
      );
      expect(
        kLevelDesign[i].xpReward,
        greaterThan(kLevelDesign[i - 1].xpReward),
      );
      expect(
        kWaveSpawnTable[i].enemyCount,
        greaterThan(kWaveSpawnTable[i - 1].enemyCount),
      );
      expect(
        kWaveSpawnTable[i].pressureBudget,
        greaterThan(kWaveSpawnTable[i - 1].pressureBudget),
      );
      expect(
        kWaveSpawnTable[i].spawnCadenceSeconds,
        lessThan(kWaveSpawnTable[i - 1].spawnCadenceSeconds),
      );
    }
  });
  ...
  test('balancing data stays aligned with the playable level roadmap', () {
    ...
    expect(
      kProgressionUnlockPlan.toSet().length,
      kProgressionUnlockPlan.length,
    );
